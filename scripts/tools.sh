...
space_info()
{
    local data_folder_info=(`df -h /opt/mannheimworld/data | sed -n '2p'`)
cat << EOF
>>>>>> Base data folder <<<<<<
Path: /opt/mannheimworld/data
...
EOF
    ...
    for i in $(seq 1 128); do
        local disk_folder_info=(`df -h /opt/mannheimworld/disks/${i} | sed -n '2p'`)
        if [ x"${disk_folder_info[0]}" != x"${data_folder_info[0]}" ]; then
            printf "\n>>>>>> Storage folder ${i} <<<<<<\n"
            printf "Path: /opt/mannheimworld/disks/${i}\n"
    ...
    if [ "$has_disks" == false ]; then
        log_err "Please mount the hard disk to storage folders, paths is from: /opt/mannheimworld/disks/1 ~ /opt/mannheimworld/disks/128"
        return 1
    fi
...
PS:
1. Base data folder is used to store chain and db, 2TB SSD is recommended, you can mount SSD on /opt/mannheimworld/data
2. Please mount the hard disk to storage folders, paths is from: /opt/mannheimworld/disks/1 ~ /opt/mannheimworld/disks/128
...
storage_ab_upgrade()
{
    ...
    if [ x"$mrenclave" == x"$code" ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            aimage=${aimage[1]}
            if [ x"$aimage" != x"mannheimworld/spacex-storage:latest" ]; then
                docker tag $aimage mannheimworld/spacex-storage:latest
            fi
        fi
        ...
        if [ $? -eq 0 ]; then
            ...
            bimage=${bimage[1]}
            if [ x"$bimage" != x"mannheimworld/spacex-storage:latest" ]; then
                docker tag $bimage mannheimworld/spacex-storage:latest
            fi
        fi
        return 0
    fi
    ...
    local old_image=(`docker images | grep '^\b'mannheimworld/spacex-storage'\b ' | grep 'latest'`)
    ...
    res=$(($?|$res))
    docker tag $docker_org/spacex-storage:$code mannheimworld/spacex-storage:latest
    ...
    local new_image=(`docker images | grep '^\b'mannheimworld/spacex-storage'\b ' | grep 'latest'`)
    ...
    if [ $resa -eq 0 ] && [ $resb -eq 0 ] ; then
        ...
        if [ $? -ne 0 ]; then
            ...
            docker tag $old_image mannheimworld/spacex-storage:latest
            return 1
        fi
    fi
    ...
    # Set new information
    docker tag $new_image mannheimworld/spacex-storage:latest
